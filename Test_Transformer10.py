...
def load_transformer_model(results_folder):
    ...
    print(f"✅ Transformer Model loaded from: {model_filename}")
    ...
    print(f"✅ Loaded feature scaling parameters.")
...
def make_transformer_predictions(model, x_test, y_test, X_mean, X_std, y_mean, y_std, input_size=5, horizon=1):
    ...
    # ✅ Standardize test data manually
    ...
    #print(f"✅ Reshaped x_test for prediction: {x_test_scaled_seq.shape}")
            ...
            for i in range(t - input_size, t):
                ...
                for col in feature_columns:
                    ...
                    if original_col is None:
                        print(f"⚠️ WARNING: Column {col} is missing in feature_mapping. Using 0 as default.")
                        normalized_value = 0  # Default to 0 if column mapping is missing
                    elif original_col not in X_mean or original_col not in X_std:
                        print(f"⚠️ WARNING: Column {col} (mapped to {original_col}) not found in X_mean/X_std. Using 0 as default.")
                        ...
                        if pd.isna(raw_value):  # Handle NaN values explicitly
                            print(f"⚠️ WARNING: NaN detected for {col} at index {i}. Using 0 as default.")
                            normalized_value = 0
                        else:
...
def main():
    ...
    # ✅ Load Transformer model and scaling parameters
