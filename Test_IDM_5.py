...
# ✅ Make Predictions using IDM
def make_predictions(x_test, y_test, model_type, input_size=10, horizon=1):
    """
    Computes acceleration using the Intelligent Driver Model (IDM), ensuring realistic predictions.
    ...
    Returns:
    - y_pred_unscaled: IDM predicted accelerations
    """
    
       # ✅ IDM Parameters
    a, exp, v0, s0, t, b = [2.009, 1.007, 19.006, 8.993, 2.183, 17.119]
    ...
    print(f"✅ Running IDM-based acceleration computation for {len(x_test)} samples...")
    ...
    for idx in range(len(x_test)):
        ...
        try:
            ...
            y_pred_unscaled.append(np.nan)  # Mark as NaN for handling later
            continue  # Skip this row
    
        # ✅ If delta_t is not 1, copy original values
        if delta_t != 1:
            x_test.at[idx, "Simulated_acc_IDM"] = x_test.at[idx, "Follower_acc"]
            x_test.at[idx, "Simulated_sp_IDM"] = x_test.at[idx, "Follower_sp_[ft]"]
            x_test.at[idx, "Simulated_pos_IDM"] = x_test.at[idx, "Follower_pos_[ft]"]
            x_test.at[idx, "Simulated_delta_s_IDM"] = x_test.at[idx, "delta_s"]
            x_test.at[idx, "Simulated_delta_v_IDM"] = x_test.at[idx, "delta_v"]
            x_test.at[idx, "Simulated_Follower_acc_t-1_IDM"] = x_test.at[idx, "Follower_acc_t-1"]
            
            print(f"🔄 Copying values for index {idx} due to delta_t ≠ 1 (delta_t = {delta_t}).")
            y_pred_unscaled.append(np.nan)  # No prediction needed
            continue  # Skip to the next iteration
    
        # ✅ Handle NaNs
        if pd.isnull([leader_speed, leader_pos, follower_speed, follower_pos]).any():
            print(f"⚠️ Warning: NaN values detected at index {idx}. Skipping.")
            y_pred_unscaled.append(np.nan)
            continue
    
        # ✅ Ensure non-negative spacing
        safe_s = max(leader_pos - follower_pos, 1.0)
    
        # ✅ Relative velocity (delta_v)
        delta_v = np.clip(leader_speed - follower_speed, -25, 25)
    
        # ✅ Desired spacing (s_d)
        sqrt_term = np.sqrt(a * b) if a > 0 and b > 0 else float('inf')
        s_d = s0 + max(1, follower_speed * t + (follower_speed * delta_v) / (2 * sqrt_term))
    
        # ✅ Compute IDM acceleration
        pred_acc = (a * (1 - np.clip((follower_speed / v0) ** exp, 0, 1) - np.clip((s_d / safe_s) ** 2, 0, 1)))
    
        # ✅ Apply acceleration constraints
        pred_acc = np.clip(pred_acc, max_desac, max_accel)
        if follower_speed >= max_speed and pred_acc > 0:
            pred_acc = 0  # No acceleration at max speed
    
        # ✅ Store the computed acceleration
        x_test.at[idx, "Simulated_acc_IDM"] = pred_acc * delta_t
        x_test.at[idx, "Simulated_sp_IDM"] = follower_speed + pred_acc * delta_t
        x_test.at[idx, "Simulated_pos_IDM"] = follower_pos + x_test.at[idx, "Simulated_sp_IDM"] * delta_t + 0.5 * pred_acc * (delta_t ** 2)
        x_test.at[idx, "Simulated_delta_s_IDM"] = leader_pos - x_test.at[idx, "Simulated_pos_IDM"]
        x_test.at[idx, "Simulated_delta_v_IDM"] = leader_speed - x_test.at[idx, "Simulated_sp_IDM"]
        x_test.at[idx, "Simulated_Follower_acc_t-1_IDM"] = pred_acc
    
        y_pred_unscaled.append(pred_acc * delta_t)  # Scaling by delta_t for step consistency
    
    # ✅ Convert to NumPy array for compatibility
    y_pred_unscaled = np.array(y_pred_unscaled).reshape(-1, 1)
    
    print("✅ IDM acceleration predictions complete.")


    return y_pred_unscaled, x_test, y_test  # Returning x_test and y_test unchanged for compatibility
...
def simulate_vehicle_motion(df_test, results_folder, model_type, input_size=10, horizon=1):
    """
    Simulates vehicle motion using the IDM model by iteratively predicting acceleration,
    updating speed, and computing position with realistic constraints.
    ...
    print(f"🚀 Starting IDM Simulation for {model_type}")
    ...
    for t in range(input_size, len(df_simulated) - horizon):
        ...
        # ✅ Store the computed acceleration
        df_simulated.at[t + horizon, f"Simulated_acc_{model_type}"] = pred_acc * delta_t
